...
const category = categories.find((item) => item.id === "tiktok")!;
...
export default function TikTokCategoryPage() {
  const categoryTools = tools.filter((tool) => tool.categoryId === "tiktok");
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="card p-8">
        <h2 className="text-2xl font-semibold text-slate-900">Why TikTok Matters for Growth</h2>
        <p className="mt-3 text-slate-600">{category.expandedIntro}</p>
        <p className="mt-4 text-slate-600">
          Use the tools below to generate hooks, captions, and script starters in seconds. Test multiple angles and
          double down on what gets the best retention and comments from your audience.
        </p>
      </section>

      <section>
        <h2 className="text-2xl font-semibold text-slate-900">TikTok Tools</h2>
        <p className="mt-2 text-sm text-slate-600">Hook lines, captions, and script starters built for retention.</p>
      ...
      <section className="card p-8">
        ...
        <ol className="mt-3 list-inside list-decimal space-y-2 text-slate-600">
          <li>Choose a tool that matches your content goal — hooks, captions, or scripts.</li>
          <li>Enter your topic or keyword and click Generate to get 5 optimized results.</li>
          <li>Pick the strongest option, customize it with your voice, and publish.</li>
          <li>Track performance (watch time, comments, shares) and generate new variations to test against winners.</li>
        </ol>
      </section>
